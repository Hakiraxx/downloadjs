...
let facebookService, instagramService, tiktokVideoService, tiktokPhotoService, soundcloudService;

try {
  facebookService = require('../services/facebookService');
  instagramService = require('../services/instagramService');
  tiktokVideoService = require('../services/tiktokVideoService');
  tiktokPhotoService = require('../services/tiktokPhotoService');
  soundcloudService = require('../services/soundcloudService');
} catch (error) {
...
app.post('/api/facebook/download', async (req, res) => {
  try {
    if (!facebookService) {
      return res.status(503).json({ error: 'Facebook service unavailable' });
    }
    const { url } = req.body;
    if (!url) {
...
app.post('/api/instagram/download', async (req, res) => {
  try {
    if (!instagramService) {
      return res.status(503).json({ error: 'Instagram service unavailable' });
    }
    const { url } = req.body;
    if (!url) {
      return res.status(400).json({ error: 'URL is required' });
...
app.post('/api/tiktok/video/download', async (req, res) => {
  try {
    if (!tiktokVideoService) {
      return res.status(503).json({ error: 'TikTok video service unavailable' });
    }
    const { url } = req.body;
    if (!url) {
      return res.status(400).json({ error: 'URL is required' });
...
app.post('/api/tiktok/photo/download', async (req, res) => {
  try {
    if (!tiktokPhotoService) {
      return res.status(503).json({ error: 'TikTok photo service unavailable' });
    }
    const { url } = req.body;
    if (!url) {
      return res.status(400).json({ error: 'URL is required' });
...
app.post('/api/soundcloud/info', async (req, res) => {
  try {
    if (!soundcloudService) {
      return res.status(503).json({ error: 'SoundCloud service unavailable' });
    }
    const { url } = req.body;
    if (!url) {
      return res.status(400).json({ error: 'URL is required' });
